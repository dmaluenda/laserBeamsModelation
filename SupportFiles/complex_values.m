...
title 'SLM1'
legend 'SLM curve' 'Possible coding values' 'Amplitude only modulation'
hold off
cd plots
print(h,'-depsc',[prefixFn 'polar_SLM1.eps']);
cd ..
...
hold off
cd plots
print(h,'-depsc',[prefixFn 'polar_SLM2.eps']);
cd ..
...
fid     = fopen([prefixFn 'complexValues_SLM2.txt'], 'wt' );
fprintf( fid , '%10.20f  %10.20f  %3.0f  %3.0f\n' , dataCM2);
fclose(fid);
% Amplitude Modulation
dataAM2 = [ abs(B2(pA2))/A2_maxAM ; phi2(pA2) ; aux2_i(pA2) ; aux2_j(pA2) ];
fid     = fopen([prefixFn 'amplitudeValues_SLM2.txt'], 'wt' );
fprintf( fid , '%10.20f  %10.20f  %3.0f  %3.0f\n' , dataAM2 );
fclose(fid);
